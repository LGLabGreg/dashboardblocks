    ...
    <DropdownMenuTrigger>
      ...
      <DropdownMenu placement='bottom end' className='w-auto min-w-44'>
        <DropdownMenuGroup
          ...
          selectedKeys={[`column:${sort.key}`]}
          onSelectionChange={(keys) => {
            const [key] = keys === 'all' ? [] : [...keys]
            if (key !== undefined) {
              onSortChange({ ...sort, key: String(key).slice('column:'.length) as K })
            }
          }}
        >
          <DropdownMenuLabel>Sort by</DropdownMenuLabel>
          {columns.map((column) => (
            <DropdownMenuItem key={column.key} id={`column:${column.key}`}>
              {column.label}
            </DropdownMenuItem>
          ))}
        </DropdownMenuGroup>
        ...
        >
          ...
          <DropdownMenuItem id='descending'>High to low</DropdownMenuItem>
          <DropdownMenuItem id='ascending'>Low to high</DropdownMenuItem>
        </DropdownMenuGroup>
      </DropdownMenu>
    </DropdownMenuTrigger>
  )
